...
describe('Database', () => {
  ...
  beforeEach(() => {
    ...
    (DatabaseSync as jest.MockedClass<typeof DatabaseSync>).mockImplementation(() => mockDb as unknown as DatabaseSync);
  ...
  describe('Singleton Pattern', () => {
    ...
    it('deve inicializar as tabelas na criação da instância', () => {
      ...
      expect(mockExec).toHaveBeenNthCalledWith(1, expect.stringContaining('CREATE TABLE IF NOT EXISTS users'));
      expect(mockExec).toHaveBeenNthCalledWith(2, expect.stringContaining('CREATE TABLE IF NOT EXISTS orders'));
      expect(mockExec).toHaveBeenNthCalledWith(3, expect.stringContaining('CREATE TABLE IF NOT EXISTS products'));
